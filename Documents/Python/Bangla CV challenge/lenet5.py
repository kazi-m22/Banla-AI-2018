...
import keras.preprocessing.image as km
...
paths_train_i = glob.glob(os.path.join(data_dir, 'training-h', '*.png'))
paths_train_all = paths_train_a + paths_train_b + paths_train_c + paths_train_d + paths_train_e + paths_train_f + paths_train_g + paths_train_h + paths_train_i
...
paths_test_f = glob.glob(os.path.join(data_dir, 'testing-f', '*.png')) + glob.glob(
    os.path.join(data_dir, 'testing-f', '*.JPG'))
...
X_train_all = np.concatenate(
    (X_train_a, X_train_b, X_train_c, X_train_d, X_train_e, X_train_f, X_train_g, X_train_h, X_train_i), axis=0)
y_train_all = np.concatenate(
    (y_train_a, y_train_b, y_train_c, y_train_d, y_train_e, y_train_f, y_train_g, y_train_h, y_train_i), axis=0)
...
def my_model():
    model = Sequential()
    # C1 Layer
    model.add(Conv2D(32, 5, 5, border_mode='same', input_shape=(28, 28, 1)))
    ...
    # Second Layer
    model.add(Conv2D(16, 5, 5, border_mode='valid', input_shape=(14, 14, 1)))
    model.add(Activation('relu'))
    # Max pooling
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.25))
    # Third Layer
    model.add(Conv2D(16, 5, 5, border_mode='valid', input_shape=(5, 5, 1)))
    ...
    model.compile(loss='categorical_crossentropy', metrics=['accuracy'], optimizer=optmzr)
    return model
model = my_model()
model.summary()
...
# model =  build_lenet(28,28,1,10,None)# create the model
model = my_model()
...
datagen = km.ImageDataGenerator(rotation_range=50,width_shift_range=.2, height_shift_range=.2,featurewise_std_normalization=True )
datagen.fit(X_train)
...
create_submission(predictions=labels,keys=keys,path='shahriai_sub_lenet5_7.csv')