...
def my_model(img_size=28, channels=1):
    ...
    model.add(Conv2D(32, (5, 5), input_shape=input_shape, activation='elu', padding='same'))
...
# model =build_lenet(28,28,1,10,None)
model = my_model()
model.summary()
...
create_submission(predictions=labels,keys=keys,path='misbah_sub.csv')